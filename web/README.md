...
| `AUTH_URL`              | —       | Auth.js base URL including the auth path (e.g. `http://localhost:3000/api/auth`). **Must use `https://` in production** — the server refuses to start otherwise (Auth.js disables Secure cookies and the `__Secure-`/`__Host-` cookie prefix when this is HTTP). |
| `AUTH_OIDC_ISSUER`      | —       | OIDC issuer URL of your IdP. Must match the senko backend's IdP. **Must use `https://` in production** — the server refuses to start otherwise (RFC 9700 §3 / OIDC Core §16.21 require TLS for OIDC discovery, token, and userinfo). |
...
| `NODE_ENV`              | —       | Set to `production` for production deployments. When `production`, the server refuses to start if `WEB_DEV_AUTH_BYPASS=true` (fail-fast assert) or if `AUTH_URL` / `AUTH_OIDC_ISSUER` are not `https://`; both bypass code paths (BFF / `_authed` gate) are also unreachable regardless of `WEB_DEV_AUTH_BYPASS`. Always set this in production. |
...
| `SENKO_AUTH_REQUIRED_SCOPE` | — | Optional sign-in gate. When set, the OAuth `access_token`'s `scope` claim (space-separated) must contain this value, otherwise the Auth.js `signIn` callback returns `false` and the user is redirected to the Auth.js error page. |
| `SENKO_AUTH_REQUIRED_GROUPS` | — | Optional sign-in gate. Comma-separated allow-list. The `access_token` claim named by `SENKO_AUTH_GROUPS_CLAIM` must intersect this list (ANY match). |
| `SENKO_AUTH_GROUPS_CLAIM` | — | Optional sign-in gate. Claim key holding the user's groups (e.g. `senko_groups`, `cognito:groups`, `groups`). The claim value is parsed as a JSON array, comma-separated string, or whitespace-separated string. **Required when `SENKO_AUTH_REQUIRED_GROUPS` is set** — otherwise startup logs a warning and every sign-in is rejected (fail-secure). |

> **Sign-in gate semantics.** All three above unset (default) → every sign-in is allowed. When any of `SENKO_AUTH_REQUIRED_SCOPE` / `SENKO_AUTH_REQUIRED_GROUPS` is set, the access_token is JWT-decoded (no signature check — the IdP already validated it) and the configured claim is checked. Both set means AND. Missing/undecodable tokens, missing claims, or `SENKO_AUTH_REQUIRED_GROUPS` without `SENKO_AUTH_GROUPS_CLAIM` all fail-secure. For a Cognito recipe (pre-token Lambda that injects `senko_groups`), see [docs/en/guides/web/aws-lambda-cognito.md](../docs/en/guides/web/aws-lambda-cognito.md) (or [docs/ja/...](../docs/ja/guides/web/aws-lambda-cognito.md)).
