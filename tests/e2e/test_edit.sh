...
ADD_OUTPUT="$(run_lf --output json task add --title "Original Title")"
TASK_ID="$(echo "$ADD_OUTPUT" | jq -r '.id')"

# 1. Scalar field changes
echo "[1] Scalar field changes"

OUT="$(run_lf --output json task edit "$TASK_ID" --title "New Title")"
assert_json_field "$OUT" '.title' "New Title" "edit title"

OUT="$(run_lf --output json task edit "$TASK_ID" --background "bg text")"
assert_json_field "$OUT" '.background' "bg text" "edit background"

OUT="$(run_lf --output json task edit "$TASK_ID" --description "description text")"
assert_json_field "$OUT" '.description' "description text" "edit description"

OUT="$(run_lf --output json task edit "$TASK_ID" --plan "plan text")"
...
OUT="$(run_lf --output json task edit "$TASK_ID" --clear-background)"
assert_json_field "$OUT" '.background' "null" "clear background"

OUT="$(run_lf --output json task edit "$TASK_ID" --clear-description)"
assert_json_field "$OUT" '.description' "null" "clear description"

OUT="$(run_lf --output json task edit "$TASK_ID" --clear-plan)"
assert_json_field "$OUT" '.plan' "null" "clear plan"

# 3. Array field operations (tags)
echo "[3] Array field operations (tags)"

OUT="$(run_lf --output json task edit "$TASK_ID" --add-tag rust --add-tag cli)"
TAGS="$(echo "$OUT" | jq -r '.tags | sort | join(",")')"
assert_eq "cli,rust" "$TAGS" "add-tag rust and cli"

OUT="$(run_lf --output json task edit "$TASK_ID" --remove-tag cli)"
TAGS="$(echo "$OUT" | jq -r '.tags | sort | join(",")')"
assert_eq "rust" "$TAGS" "remove-tag cli"

OUT="$(run_lf --output json task edit "$TASK_ID" --set-tags new1 new2)"
TAGS="$(echo "$OUT" | jq -r '.tags | sort | join(",")')"
...
OUT="$(run_lf --output json task edit "$TASK_ID" --remove-definition-of-done "item1")"
...
run_lf --output json task edit "$TASK_ID" --remove-definition-of-done "only1" >/dev/null

# 5. Array field operations (in_scope)
echo "[5] Array field operations (in_scope)"

OUT="$(run_lf --output json task edit "$TASK_ID" --add-in-scope "scope1" --add-in-scope "scope2")"
IN_SCOPE="$(echo "$OUT" | jq -c '.in_scope | sort')"
assert_eq '["scope1","scope2"]' "$IN_SCOPE" "add-in-scope scope1 and scope2"

OUT="$(run_lf --output json task edit "$TASK_ID" --remove-in-scope "scope1")"
IN_SCOPE="$(echo "$OUT" | jq -c '.in_scope')"
assert_eq '["scope2"]' "$IN_SCOPE" "remove-in-scope scope1"

OUT="$(run_lf --output json task edit "$TASK_ID" --set-in-scope new_s1 new_s2)"
IN_SCOPE="$(echo "$OUT" | jq -c '.in_scope | sort')"
...
OUT="$(run_lf --output json task edit "$TASK_ID" --add-out-of-scope "out1" --add-out-of-scope "out2")"
OUT_SCOPE="$(echo "$OUT" | jq -c '.out_of_scope | sort')"
assert_eq '["out1","out2"]' "$OUT_SCOPE" "add-out-of-scope out1 and out2"

OUT="$(run_lf --output json task edit "$TASK_ID" --remove-out-of-scope "out1")"
OUT_SCOPE="$(echo "$OUT" | jq -c '.out_of_scope')"
assert_eq '["out2"]' "$OUT_SCOPE" "remove-out-of-scope out1"

OUT="$(run_lf --output json task edit "$TASK_ID" --set-out-of-scope new_o1 new_o2)"
OUT_SCOPE="$(echo "$OUT" | jq -c '.out_of_scope | sort')"
...
OUT="$(run_lf --output json task edit "$TASK_ID" --plan-file "$PLAN_FILE")"
...
# 7b. File not found
assert_exit_code 1 run_lf --output json task edit "$TASK_ID" --plan-file "/nonexistent/plan.md"

# 7c. --plan and --plan-file conflict (clap error → exit 2)
assert_exit_code 2 run_lf --output json task edit "$TASK_ID" --plan "inline" --plan-file "$PLAN_FILE"

# 8. Non-existent task ID
echo "[8] Non-existent task ID"
assert_exit_code 1 run_lf --output json task edit 9999 --title "X"
